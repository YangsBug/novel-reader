...
function generateContent() {
  const count = 4 + randomInt(5);
  let content = '';
  for (let i = 0; i < count; i++) {
    content += PARAGRAPHS[randomInt(5)] + '\n\n';
  }
  return content;
}
...
async function main() {
  ...
  const [novels] = await conn.query('SELECT id FROM novels');
  let totalChapters = 0;

  for (const novel of novels) {
    const chapterCount = 50 + randomInt(51);
    const values = [];

    for (let i = 1; i <= chapterCount; i++) {
      const title = `第${i}章 ` + CHAPTER_TITLES[randomInt(20)];
      const content = generateContent();
      const wordCount = content.length;
      values.push([novel.id, title, content, wordCount, i]);
    }
    ...
    await conn.query(
      `INSERT INTO chapters (novel_id, title, content, word_count, chapter_no) VALUES ${placeholders}`,
      flatValues
    );

    // 更新小说字数
    await conn.query(
      'UPDATE novels SET word_count = (SELECT COALESCE(SUM(word_count),0) FROM chapters WHERE novel_id = ?) WHERE id = ?',
      [novel.id, novel.id]
    );

    totalChapters += chapterCount;
    console.log(`  [${novel.id}] 已生成 ${chapterCount} 章`);
  }

  await conn.end();
  console.log(`\n✅ 完成！共生成 ${totalChapters} 章（25本小说）`);
}
