...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin) {
      if (!confirmPassword) { toast.error('请确认密码'); return }
    ...
    try {
      ...
      toast.error(e.message || (isLogin ? '登录失败' : '注册失败'))
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="card p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input value={password} onChange={e => setPassword(e.target.value)} type="password" className="input-field" placeholder="输入密码（至少6位）" autoComplete={isLogin ? 'current-password' : 'new-password'} />
            </div>
            {!isLogin && (
              <div>
                <label className="text-xs mb-1 block" style={{ color: 'var(--text-muted)' }}>确认密码</label>
                <input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type="password" className="input-field" placeholder="再次输入密码" autoComplete="new-password" />
              </div>
            )}
          ...
          <div className="text-center text-[10px] mt-6" style={{ color: 'var(--text-secondary)' }}>
            <p>注册即用，无需邮箱验证</p>
          </div>
        </div>
      </motion.div>
