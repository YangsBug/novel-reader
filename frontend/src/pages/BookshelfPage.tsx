...
export default function BookshelfPage() {
  ...
  const { data: books, isLoading } = useQuery({
    ...
    queryFn: () => bookshelfApi.list(tab || undefined).then(r => r.data.data),
    enabled: isLoggedIn,
  })
  ...
  const removeMut = useMutation({
    mutationFn: (novelId: number) => bookshelfApi.remove(novelId),
      ...
      {isLoading ? <Loading /> : !books?.length ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" style={{ perspective: '1200px' }}>
          {books.map((book, i) => (
            ...
            >
              <div
                className="card p-4 flex gap-4 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl"
                style={{
              ...
              >
                <Link to={`/novel/${book.novelId}`} className="shrink-0">
                  <div className="w-20 h-28 rounded-lg overflow-hidden bg-slate-200 transition-shadow duration-300"
                    style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
                    {book.novelCover ? (
                      <img src={book.novelCover} alt={book.novelTitle} className="w-full h-full object-cover"
                        onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                    ) : null}
                  </div>
                </Link>
                <div className="flex-1 min-w-0">
                  <Link to={`/novel/${book.novelId}`} className="text-sm font-medium transition-colors line-clamp-1"
                    style={{ color: 'var(--text-primary)' }}
                    onMouseEnter={e => e.currentTarget.style.color = 'var(--accent-primary)'}
                    onMouseLeave={e => e.currentTarget.style.color = 'var(--text-primary)'}>{book.novelTitle}</Link>
                  ...
                  <div className="flex gap-2 mt-3">
                    {book.latestChapterNo && (
                      <Link to={`/reader/${book.novelId}/${book.latestChapterNo}`} className="text-[10px] btn-primary py-1 px-3">继续阅读</Link>
                    )}
                    <button onClick={() => removeMut.mutate(book.novelId)} className="text-[10px] text-red-500 py-1 px-2 hover:opacity-70">移除</button>
                  </div>
                </div>
              </div>
            </motion.div>
